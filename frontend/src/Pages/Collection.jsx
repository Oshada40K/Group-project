...
const Collection = () => {
  ...
  // Apply filter when category, subCategory, or search changes
  useEffect(() => {
    applyFilter();
        ...
        <div className='grid grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-4 gap-y-6'>
          {filterProducts.map((item, index) => (
            <ProductItem 
              ...
              id={item._id} 
              price={item.price} 
              image={Array.isArray(item.Image) && item.Image.length > 0 ? item.Image[0] : item.Image} // FIX: Show first image if multiple
            />
          ))}
        </div>
      </div>
